...
export default function BoletimScreen() {
  ...
  // Carrega toda a base de dados necessária para a visão global do Admin
  const carregarDados = async () => {
    setLoading(true);
    try {
      const [resCursos, resDisc, resAlunos, resProfs, resNotas] = await Promise.all([
  ...
  const salvarEdicao = async () => {
    ...
    try {
      ...
      Alert.alert('Sucesso', `Nota do aluno atualizada! Nova média: ${mediaCalc.toFixed(1)}`);
      carregarDados(); 
    } catch (error) {
  ...
  // Filtros em Cascata
  const disciplinasDoCurso = disciplinas.filter(d => d.curso === cursoSelecionado?.nome);
  ...
  const professorResponsavel = professores.find(p => p.id === disciplinaSelecionada?.professor_id);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Gestão Geral de Notas</Text>
      ...
      >
        <Text style={{ color: cursoSelecionado ? '#333' : '#999', fontWeight: 'bold' }}>
          {cursoSelecionado ? `Curso: ${cursoSelecionado.nome}` : '1. Selecione o Curso'}
        </Text>
      </TouchableOpacity>
      ...
      {cursoSelecionado && (
        ...
        >
          <Text style={{ color: disciplinaSelecionada ? '#333' : '#999', fontWeight: 'bold' }}>
            {disciplinaSelecionada ? `Disciplina: ${disciplinaSelecionada.nome}` : '2. Selecione a Disciplina'}
          </Text>
        </TouchableOpacity>
      )}
      ...
      {showDisciplinaDropdown && (
        <View style={styles.dropdownContainer}>
          {disciplinasDoCurso.length === 0 && <Text style={{padding: 10}}>Nenhuma disciplina para este curso.</Text>}
      ...
      {/* 3. INFORMAÇÃO DO PROFESSOR */}
      {disciplinaSelecionada && (
        <View style={styles.professorContainer}>
          <Text style={styles.professorText}>
            👨‍🏫 Prof. Responsável: <Text style={{fontWeight: 'bold'}}>{professorResponsavel ? professorResponsavel.nome : 'Não atribuído'}</Text>
          </Text>
        </View>
      )}
      ...
      {loading ? (
        ...
        <Text style={styles.emptyText}>Siga os passos acima para visualizar as notas.</Text>
      ) : notasExibidas.length === 0 ? (
        <Text style={styles.emptyText}>Nenhuma nota lançada para esta disciplina ainda.</Text>
      ) : (
        <FlatList
          data={notasExibidas}
      ...
      <Modal visible={modalVisible} transparent={true} animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Editar Notas (Admin)</Text>
            <Text style={styles.modalSubtitle}>Digite a nova pontuação.</Text>
...
const styles = StyleSheet.create({
  ...
  dropdownToggle: { backgroundColor: '#fff', padding: 15, borderRadius: 8, borderWidth: 1, borderColor: '#ddd', marginBottom: 10, elevation: 1 },
  dropdownContainer: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, marginBottom: 15, elevation: 2, maxHeight: 150, overflow: 'hidden' },
  dropdownItem: { padding: 15, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  dropdownItemText: { fontSize: 16, color: '#333' },
  
  professorContainer: { backgroundColor: '#e9ecef', padding: 12, borderRadius: 8, marginBottom: 15, borderWidth: 1, borderColor: '#ced4da' },
  professorText: { color: '#495057', fontSize: 15 },
  ...
  notasRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15, backgroundColor: '#f8f9fa', padding: 10, borderRadius: 8 },
  ...
  editButton: { backgroundColor: '#007bff', paddingVertical: 8, paddingHorizontal: 15, borderRadius: 8 }, // Azul para indicar ação de admin
