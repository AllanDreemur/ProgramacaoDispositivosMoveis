...
// Publicar novo aviso
exports.cadastrarAviso = async (req, res) => {
  const { titulo, mensagem, autor } = req.body;
  try {
    if (!titulo || !mensagem || !autor) {
      return res.status(400).json({ error: 'Todos os campos são obrigatórios.' });
    }
    const query = `INSERT INTO avisos (titulo, mensagem, autor) VALUES ($1, $2, $3)`;
    await pool.query(query, [titulo, mensagem, autor]);
...
// Listar todos os avisos (do mais recente para o mais antigo)
